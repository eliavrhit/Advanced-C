...
void main()
{
	...
	arr_even = (int*)calloc(N, sizeof(int));
	arr_odd = (int*)calloc(N, sizeof(int));
	if (!arr_even || !arr_odd)
	{
	...
	for (i = 0; i < N; i++)
		if (a[i] % 2 == 0)
			...
			arr_odd[odd++] = a[i];
	arr_even = (int*)realloc(arr_even, even * sizeof(int));
	arr_odd = (int*)realloc(arr_odd, odd * sizeof(int));
